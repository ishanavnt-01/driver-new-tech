...
from black_spots.tasks.generate_training_input import get_training_noprecip
...
def get_latest_segments_tar_uuid(roads_srid, records_csv_obj_id):
    ...
    lines_shp_path = load_road_network.load_road_network(output_srid='EPSG:{}'.format(roads_srid))
    logger.info('----lines_shp_path---{}---------'.format(lines_shp_path))
...
@shared_task
def calculate_black_spots(history_length=datetime.timedelta(days=10), roads_srid=4326):
    ...
    logger.info(f'------records_csv_obj_id----{records_csv_obj_id}---')
