...
def get_data(records_csv_uuid, segments_shp_uuid, road_srid):
    ...
    with BlackSpotRecordsFile.objects.get(uuid=records_csv_uuid).csv as records_csv:
        records_csv.open('r')  # Open in text mode, not binary
...
def write_black_spot_training_csv(segments_with_data, schema):
    ...
    with tempfile.SpooledTemporaryFile(mode='w+b') as csv_file:  # Ensure the file is opened in binary write mode
        ...
        # Reset file pointer to the start so it can be read into a BlackspotTrainingCsv
        csv_file.seek(0)

        # Save the file properly using Django's File API
        training_csv = BlackSpotTrainingCsv.objects.create()
        training_csv.csv.save(f'{training_csv.uuid}.csv', File(csv_file))  # Pass the file-like object here
...
def read_records(records_file, road_projection, record_projection, tz, col_id, col_x, col_y,
    ...
    # csv_reader = csv.DictReader((line.decode('utf-8').strip() for line in records_file))
    ...
    for row in csv_reader:
        ...
        records.append({
            'id': row[col_id],
            'point': transform(project, Point(float(row[col_x].strip().replace("b'", "").replace("'", "")),
                                  float(row[col_y].strip().replace("b'", "").replace("'", "")))),
            'occurred': occurred,
