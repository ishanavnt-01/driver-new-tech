...
def create_schema(schema_path, api, headers=None):
    ...
    response = requests.get('http://'+str(Site.objects.get_current())+'data-api' + '/recordtypes/?label=Incident&active=True',
                            headers=headers)
...
# def main(**args_dict):
    ...
    csv_file = args_dict["csvpath"]
    logger.info("Loading file: {}".format(csv_file))
    # import ipdb;ipdb.set_trace()
    for record in extract(csv_file):
        if count % 100 == 0:
            logger.info("{0} {1})".format(count, csv_file))
        load(transform(record, schema_id), api_url, headers)
        count += 1
