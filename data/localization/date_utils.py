...
def hijri_day_range(min_date, max_date, periodic=False):
    ...
    years = range(int(min_date_um.year), int(max_date_um.year) + 1)
    days = []
    if periodic:
        days = [
            HijriDateSet(
    ...
    for year in years:
        ...
        for month in range(int(min_date_um.month), max_month + 1):
            ...
            for day in range(int(start_date_um.day), int(last_date_um.day) + 1):
    ...
    if periodic:
        months = [
            ...
            for month, month_num in [
                    (HijriDate(1437, x, 1).month_name.decode('utf-8'), x) for x in range(1, 13)
            ]
        ]
        min_month = 1
...
def hijri_year_range(min_date, max_date):
    ...
    max_date_um = gregorian_to_hijri(max_date)
    bounds = DateRange(start=min_date, end=max_date)
    hijri_years = range(int(min_date_um.year), int(max_date_um.year) + 1)
    years = []
